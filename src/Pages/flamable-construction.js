...
const FlamableConstruction = () => {
  ...
  // State to manage three separate search fields
  ...
  // Handle tab change and clear search fields
  ...
  // Filter function with three separate search criteria
  const filterData = (data) => {
    return data.filter((item) => {
      ...
      const matchesThickness =
        !searchThickness.trim() ||
        item.coreThickness
          ?.toLowerCase()
          .includes(searchThickness.toLowerCase());
  ...
  const filteredFloorData = useMemo(
    () => filterData(floor),
    [searchFileName, searchThickness, searchDescription, floor]
  );
  ...
  // Render table rows for floor-at-grade tab
  const renderFloorTableRows = (data) => {
    if (data.length === 0) {
      return (
        <tr className="dlinfo">
  ...
  // Render static table rows for other tabs
  const renderStaticTableRows = () => {
    const staticData = [
      { isolated: true },
      { isolated: true },
      { isolated: true },
    ...
    return staticData.map((item, index) => (
      <tr className="dlinfo" key={index}>
        ...
        <td className="dlinfo hover01">
          <span className="status with-bricks">{t("with_bricks")}</span>
        </td>
        <td className="dlinfo hover01">
          <span className={`status ${item.isolated ? "" : "not-isolated"}`}>
            {item.isolated ? t("isolated") : t("not_isolated")}
          </span>
        </td>
            ...
            <div className="search-penal bg-penal">
              <div className="left-penal">
                <div className="form-group w-30">
                  <label htmlFor="searchFileName" className="form-label">
                    {t("file_name")}
                  </label>
                  <input
                    type="text"
                ...
                <div className="form-group w-40">
                  <label htmlFor="searchThickness" className="form-label">
                    {t("thickness")}
                  </label>
                  <input
                    type="text"
              ...
              <div className="right-penal">
                <a className="btn-primary" onClick={handleClearSearch}>
                  {t("search")}
            ...
            <section className="white-bg pt-0">
              ...
              <table className="dltrc" style={{ background: "none" }}>
                <tbody>
                  ...
                  {renderFloorTableRows(filteredFloorData)}
                </tbody>
              </table>
            </section>
          )}
